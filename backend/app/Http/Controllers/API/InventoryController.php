...
class InventoryController extends Controller
{
   ...
   //Mètode per evolucionar un xuxemon
    public function evolve(int $slotId): JsonResponse
    {
        $user = Auth::guard('api')->user();

        //Otenir el slot d'inventari i el xuxemon associat
        $slot = Inventory::with('xuxemon')
            ->where('user_id', $user->id)
            ->find($slotId);

        if (!$slot || !$slot->xuxemon) {
            return response()->json(['error' => 'Registre d\'inventari o xuxemon no trobat.'], 404);
        }

        //Determina la següent mida d'evolució segons la mida actual del xuxemon
        ...
        //Si el xuxemon ja és gran, retorna un error indicant que no pot evolucionar més
        if (!$nextSize) {
            return response()->json(['error' => 'Aquest xuxe ja és gran i no pot evolucionar.'], 422);
        }

        //Si no hi ha prou unitats per evolucionar, retorna un error indicant quantes unitats hi ha disponibles
        if ($slot->quantity < 3) {
            return response()->json(['error' => "Necessites 3 unitats. Tens {$slot->quantity}."], 422);
        }

        //Redueix la quantitat del slot actual en 3 unitats. Si la quantitat arriba a 0, elimina el slot, sinó, guarda els canvis
        $slot->quantity -= 3;
        if ($slot->quantity === 0) {
            $slot->delete();
        } else {
